...
def get_graph_from_row(row, output_graph_dir, model):
    from superconductors_3D.machine_learning.own_libraries.models.GNN.GNN_utils import save_graph, \
        get_graph_from_structure
...
def generate_features(input_csv_data, output_graph_dir, output_csv_data, excluded_output_csv_data, rcut, nmax, lmax, sigma, crossover, n_pca_components, n_cpus, symprec=0.01):
    ...
    # graphs
    try:
        df, df_excluded = get_graphs(df, output_graph_dir, n_cpus)
    except ModuleNotFoundError as e:
        # If MEGNet is not found, skip graph calculation and simulate same form of output dataframes.
        print('Cannot calculate graphs:', e)
        df_excluded = pd.DataFrame(columns=list(df.columns) + ['Reason for exclusion']) # empty excluded df
    
    # MAGPIE
    try:
        df = get_MAGPIE_features(df)
    except ModuleNotFoundError as e:
        print('Cannot calculate MAGPIE features:', e)

    # SOAP
    try:
        df = calculate_SOAP_features(df, rcut, nmax, lmax, sigma, crossover, n_pca_components, n_cpus, symprec)
    except (ModuleNotFoundError, ImportError) as e:
        print('Cannot make DSOAP features or PCA of DSOAP features:', e)
    ...
    write_to_csv(df, output_csv_data, comment)
...
if __name__ == '__main__':
    ...
    input_csv_data = projectpath(datadir, 'intermediate', database, f'4_SC_{database}_synthetically_doped.csv')
    
    output_graph_dir = os.path.join(datadir, 'intermediate', database, 'graphs')
    output_csv_data = projectpath(datadir, 'intermediate', database, f'5_features_SC_{database}.csv')
    ...
    generate_features(input_csv_data, output_graph_dir, output_csv_data, excluded_output_csv_data, rcut, nmax, lmax, sigma, crossover, n_pca_components, n_cpus)
