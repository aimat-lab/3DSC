...
def check_if_plots_are_as_old_plots(save_plot_dir):
    """Only for refactoring. Checks if the plots after refactoring are the same as the ones in the paper.
    """
    old_dir = f'/home/timo/superconductors_3D/analysis/dataset_statistics/220516_SC_{database}_matches'
    cmp = filecmp.dircmp(save_plot_dir, old_dir)
    print('Different files:')
    print(cmp.diff_files)

def get_filename(title, paper=False):
    filename = title + '.png'
    if paper:
        relpath = 'others'
    else:
        relpath = 'plots_for_paper'
...
def make_3DSC_statistics_plots(dataset_csv, save_plot_dir, database):
    ...
    paper_dir = os.path.join(save_plot_dir, 'plots_for_paper')
    others_dir = os.path.join(save_plot_dir, 'others')
    ...
    plt.yscale('log')
    savefile = get_filename('totreldiff_hist', paper=True)
    plt.savefig(os.path.join(save_plot_dir, savefile), dpi=300)
    plt.show()
    ...
    plt.xlabel('# crystal structures per SuperCon entry')
    plt.ylabel('# SuperCon entries')
    plt.yscale('log')
    ...
    plt.xlabel('# different space groups')
    plt.ylabel('# SuperCon entries')
    plt.yscale('log')
    ...
    plt.xlabel('relative factor of chemical formulas')
    plt.ylabel('Count')
    plt.yscale('symlog')
    plt.tight_layout()
    savefile = get_filename('chem_formulas_factors_hist', paper=True)
    plt.savefig(os.path.join(save_plot_dir, savefile), dpi=300)
    plt.show()
    ...
    savefile = get_filename('elemental_prevalence_hist', paper=True)
    ...
    if database == 'ICSD':
        ...
        max_temp = 300
        sns.histplot(x=temp, binrange=(0,max_temp))
        plt.yscale('log')
    ...
    for df0, name in zip([df_groups, df_sc_groups], ['crystals', 'SuperCon']):
        ...
        for group in groups + ['Total']:
            ...
            plt.xlabel('$T_\mathrm{c}$ (K)')
            plt.tight_layout()
            savefile = get_filename(f'tc_hist_{name}_{group}', paper=True)
            plt.savefig(os.path.join(save_plot_dir, savefile), dpi=300)
            plt.show()
...
if __name__ == '__main__':
    ...
    dataset_csv = f'/home/timo/superconductors_3D/data_before/final/{database}/SC_{database}_matches.csv'#projectpath('data', 'final', database, f'SC_{database}_matches.csv') # TODO
    ...
    check_if_plots_are_as_old_plots(save_plot_dir)
